...
This revision (v1.2.0) extends the composition contract with the API-kind-routed adapter binding of §3.3: the Composition Root's model-discovery closure and `ProviderAdapterBinding` read the per-provider recorded API kind (the OpenAI-compatible chat-completions family or the Gemini family) at the `providerAPIKind.<canonical>` key (`DES-011` §3.11) and construct the provider adapter of the recorded family — `OpenAICompatibleProviderAdapter` or `GeminiProviderAdapter` (DES-010 §3.6, §3.10) — bound to the same recorded endpoint and stored credential reference; a provider with no recorded kind resolves to the OpenAI-compatible default, so connections configured before the kind was recorded keep serving unchanged (ARC-004). The extension is additive and backward-compatible (§6.3); the binding behavior for OpenAI-compatible connections is unchanged. The new composition behavior is frozen in §3.3 and is the single source of truth for the implementation of the revision.
...
4. **Application services** — `WorkspaceService`, `ConversationService`, `ProviderConnectionService`, `ConfigurationService`, and `SendMessageUseCase` of DES-011 §3.2–§3.5, §3.8, §3.9, §3.10, §3.11, over the injected Domain contracts.
5. **Runtime provider adapter binding** — the family-routed binding of §3.3: the provider adapter of the connection's recorded API kind (`OpenAICompatibleProviderAdapter` or `GeminiProviderAdapter`), bound to the recorded endpoint and credential reference and delivered as the capability contract the send-message use case consumes (DES-011 §3.3), together with the config-driven offered-models closure `preferredModels` delivered to both `ProviderSelectionService` and the binding (DES-013 §3.3, DES-011 §3.10).
...
- **Purpose**: the mechanism by which a configured provider connection is mapped to a bound provider adapter of its recorded API family — `OpenAICompatibleProviderAdapter` or `GeminiProviderAdapter` — its endpoint and stored credential reference — and delivered to the send-message use case as the capability contract it consumes (`PRD-008`, DES-010 §3.6, §3.9, §3.10, DES-011 §3.3).
...
- The Composition Root reads, per configured provider connection, four typed configuration values at the provider-settings level (DES-009 §3.6): the stored credential reference — recorded by `ProviderConnectionService.configure(_:)` (DES-011 §3.4) — the stored endpoint — recorded by `ProviderConnectionService.updateEndpoint(_:for:)` (DES-011 §3.9) — the stored model — recorded by `ProviderConnectionService.configure(_:endpoint:model:)` or `updateModel(_:for:)` (DES-011 §3.10) — and the recorded API kind — recorded by `ProviderConnectionService.configure(_:endpoint:model:apiKind:)` or `updateAPIKind(_:for:)` (DES-011 §3.11).
- Each value is addressed by a documented, stable key derived from the provider identity's canonical string — a `ConfigurationKey<CredentialReference>`, two `ConfigurationKey<String>` values for the endpoint and the model, and a `ConfigurationKey<ProviderAPIKind>` for the API kind at the provider-settings level — so the writer (the settings surface) and the reader (the Composition Root) never diverge (DES-011 §3.9, §3.10, §3.11, DES-004).
...
- For the provider the user's selection resolves (DES-009 §3.2, DES-011 §3.3), the Composition Root reads the recorded API kind at `ProviderConnectionService.apiKindKey(for:)` — resolving to the OpenAI-compatible default when none is recorded (ARC-004) — and constructs the adapter of that family through its public initializer: `OpenAICompatibleProviderAdapter(endpoint:credential:credentialStorage:)` for the OpenAI-compatible family, `GeminiProviderAdapter(endpoint:credential:credentialStorage:)` for the Gemini family (DES-010 §3.6, §3.10). The endpoint is parsed from the recorded URL string, the credential reference is as stored, and the adapter is delivered as the `StreamingContract`/`TextGenerationContract` the send-message use case consumes (DES-010 §3.6, DES-009 §3.11.3, DES-011 §3.3). The request's `ModelReference` is the provider's recorded model when one is recorded, or the caller's requested model unchanged; `CapabilityMapping` (or the Gemini `GeminiMapping`) writes that name as the wire `model` (DES-010 §3.9.2, §3.10).
- The adapter is constructed through its public initializer over the default transport (DES-010 §3.6, §3.10); a configured connection with no recorded endpoint or no stored credential reference is not bindable and MUST surface the provider as unavailable through the Domain discovery channel — never a launch failure, never silent (DES-009 §3.2, DES-010 §3.9.3).

Normative statements:

- The binding MUST resolve the endpoint, the credential reference, the model, and the API kind from the provider-settings configuration — the exact values the settings surfaces record (DES-011 §3.4, §3.9, §3.10, §3.11) — never from untyped, raw, or global values (DES-009 §3.6, DES-004).
...
- The adapter MUST be constructed through its public initializer over the default transport, and MUST be the adapter of the provider's recorded API family — `OpenAICompatibleProviderAdapter` or `GeminiProviderAdapter` (DES-010 §3.6, §3.10); a provider with no recorded API kind MUST resolve to the OpenAI-compatible default, so connections configured before the kind was recorded keep serving unchanged (DES-011 §3.11, ARC-004). The endpoint is the recorded URL string parsed at the boundary, and the credential is the stored reference — the raw secret is resolved only when a request is built, never in composition (DES-010 §3.9.3, `ARC-005`).
