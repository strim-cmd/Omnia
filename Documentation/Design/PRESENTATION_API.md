...
| Settings | `ProviderConnectionService` and `ConfigurationService` (`DES-011` §3.4, §3.5) — including the v1.1.0 endpoint surface and the v1.2.0 model surface of `ProviderConnectionService` (`DES-011` §3.9, §3.10). |
...
The v1.2.0 revision phases extend the realization:

### Phase 9 — Model Field and Edit Model Surface

Order: the model surface of §3.4 — the provider connection form's optional Model field, the model-edit intent beside Edit Endpoint, the model editor (a sibling of the endpoint editor), and the `SettingsState.ModelEditing` condition of §3.2 — recording and resolving the model through the frozen `ProviderConnectionService` model surface (`DES-011` §3.10); the model-edit condition and the surface's configure-with-model and update-model behavior are tested on the Linux build environment.

### Phase 10 — Revision Verification

The revision is verified against the same completion criteria as Phase 5: the new flows match §3.2 and §3.4 exactly; the v1.0.0 and v1.1.0 surfaces are unchanged; the platform-independent flow logic is testable on the Linux build environment; the view-layer signature changes are confined to the SwiftUI layer and consumed only by OmniaApp's `RootView` (§3.7); and no forbidden dependency is imported (`ARC-002`, `ARC-006`, `ARC-009`).
