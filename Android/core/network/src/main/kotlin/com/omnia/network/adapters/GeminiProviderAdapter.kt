...
/**
 * Gemini (Generative Language API) provider adapter. Wires the transport seam
 * and credential storage to the Domain capability contracts.
 *
...
class GeminiProviderAdapter(
    ...
    private val client = GeminiClient(transport, credentialStorage)

    override suspend fun generateText(request: TextGenerationRequest): TextGenerationResponse {
        val wireRequest = GeminiMapping.textGenerationRequest(request.model.name, request.prompt)
        val response = try {
            client.generateContent(wireRequest, request.model.name, endpoint, credential)
        } catch (e: ProviderTransportError) {
            throw ProviderErrorMapping.capabilityError(e)
        }
        return TextGenerationResponse(text = GeminiMapping.textResponse(response))
    }

    override suspend fun sendMessage(request: ConversationRequest): ConversationResponse {
        val wireRequest = GeminiMapping.conversationRequest(
        ...
        return ConversationResponse(message = GeminiMapping.conversationResponseMessage(response))
    }

    override suspend fun stream(request: StreamingRequest): Flow<StreamingUpdate> {
        val wireRequest = GeminiMapping.streamingRequest(
            request.model.name, request.history, request.resolvedAttachments
        )
        val eventStream = try {
            client.streamGenerateContent(wireRequest, request.model.name, endpoint, credential)
        } catch (e: ProviderTransportError) {
            throw ProviderErrorMapping.capabilityError(e)
        }
        ...
        return flow {
            var partialContent = ""
            try {
                eventStream.collect { response ->
                    val update = GeminiMapping.streamingUpdate(response, identity) ?: return@collect
