...
next_tasks:
  ...
  - Continue Infrastructure Sprint 2 (milestone #7): the concrete capability surface is frozen (DES-010 v1.1.0, issue #71) and the capability mapping is implemented (issue #72); implement the concrete provider capabilities per PRD-005 — the text generation capability (issue #73), then the conversation capability (issue #74), then the streaming capability (issue #75), then the package verification (issue #76) against the frozen §3.9 surface
