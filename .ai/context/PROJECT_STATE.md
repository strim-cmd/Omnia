...
milestones:
  ...
  Application Sprint 1 – Implementation:
    ...
    Outcome:
      ...
      - Stage 2c in progress: the send-message use case is implemented; the provider connection service and the configuration service follow.
...
next_tasks:
  ...
  - Application Sprint 1 Stage 2c complete (2026-08-05): the send-message use case implemented and verified on the Linux build — SendMessageUseCase over the streaming capability contract, the ProviderSelectionService, and the ConversationRepository (issue #93); next is Stage 2d: the provider connection application service — configure, list, and remove provider connections over the ProviderRepository, the CredentialStorageProtocol, and the configuration repository, credentials stored by reference, never the secret (issue #94)
