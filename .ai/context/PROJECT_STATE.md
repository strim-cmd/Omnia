...
milestones:
  ...
  Application Sprint 1 – Implementation:
    ...
    Outcome:
      ...
      - Stage 2a in progress: the value objects and errors are implemented; the services follow in Stage 2b onward.
...
next_tasks:
  ...
  - Application Sprint 1 Stage 2a complete (2026-08-05): the application value objects and error taxonomy implemented and verified on the Linux build — SendMessageRequest, ConfigureProviderRequest, ApplicationValidationError (issue #91); next is Stage 2b: the conversation application service — create, load/select, list by workspace membership, and delete over the frozen ConversationRepository and the workspace membership (issue #92)
