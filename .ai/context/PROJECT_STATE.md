...
milestones:
  ...
  Application Sprint 1 – Implementation:
    ...
    Outcome:
      ...
      - Stage 2b in progress: the conversation service is implemented; the send-message use case, provider connection service, and configuration service follow.
...
next_tasks:
  ...
  - Application Sprint 1 Stage 2b complete (2026-08-05): the conversation application service implemented and verified on the Linux build — ConversationService over the Domain ConversationRepository and the workspace membership (issue #92); next is Stage 2c: the send-message use case — the streaming orchestration flow over the streaming capability contract, the selection service, and the conversation repository (issue #93)
