// OmniaiOSApp — the minimal iOS host application (RE-4, PRD-009).
//
// The distribution shell for iOS: the SwiftUI @main App that runs AppLaunch
...
// §3.5, DES-012 §3.5, §3.6). It is the same thin shell as the macOS host on the
// iOS App lifecycle, at the NavigationStack root view's deployment target
// (iOS 16); the AppLaunch/RootView contract is the single source of truth, so
// the shells cannot drift.
//
...
/// The iOS app shell (DES-013 §3.5): the SwiftUI `@main` entry point that
