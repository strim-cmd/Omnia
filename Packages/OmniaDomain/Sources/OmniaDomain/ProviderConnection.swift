...
///
/// A provider is an external service that can deliver capabilities. The
/// connection records what the provider declares — identity, capabilities,
/// metadata, limits, and versioning. Live availability is discovered and
/// reported by the Infrastructure layer, never by the Domain (ARC-004
/// Capability Discovery, DES-009 §3.1). Authentication is realized by
/// credential reference; the connection MUST NOT contain credentials (ARC-004,
/// ARC-005). The `Provider` aggregate builds its lifecycle on this declaration.
///
