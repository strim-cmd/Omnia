...
final class ProviderIdentityTests: XCTestCase {
    ...
    func testCreation_ProducesWellFormedCanonicalForm() {
        for _ in 0..<32 {
            XCTAssertTrue(isWellFormedCanonical(ProviderIdentity().canonicalString))
        }
    }

    func testCreation_ProducesRestorableIdentifiers() throws {
        for _ in 0..<16 {
            let identifier = ProviderIdentity()
            let restored = try XCTUnwrap(ProviderIdentity(restoring: identifier.canonicalString))
    ...
    func testEquality_SameUnderlyingValueIsEqual() throws {
        ...
        let b = try XCTUnwrap(ProviderIdentity(restoring: canonicalProviderA))
        XCTAssertEqual(a, b)
    }
    ...
    func testEquality_DifferentConceptsAreNeverEqual() throws {
        let provider = try XCTUnwrap(ProviderIdentity(restoring: canonicalProviderA))
        let credential = try XCTUnwrap(CredentialReference(restoring: canonicalProviderA))
        XCTAssertNotEqual(AnyHashable(provider), AnyHashable(credential))
    }
    ...
    func testHashability_DictionaryKeysAndSetMembers() throws {
        let a = try XCTUnwrap(ProviderIdentity(restoring: canonicalProviderA))
        let b = try XCTUnwrap(ProviderIdentity(restoring: canonicalProviderB))
        let c = try XCTUnwrap(ProviderIdentity(restoring: canonicalProviderA))

        var dictionary: [ProviderIdentity: String] = [:]
        dictionary[a] = "alpha"
        dictionary[b] = "beta"
        ...
        let set: Set<ProviderIdentity> = [a, b, c]
    ...
    func testUniqueness_GeneratedIdentifiersAreDistinct() {
        var generated: Set<ProviderIdentity> = []
        for _ in 0..<1_000 {
            generated.insert(ProviderIdentity())
        }
        XCTAssertEqual(generated.count, 1_000)
    }

    func testUniqueness_GeneratedIdentifiersDifferFromRestored() throws {
        let restored = try XCTUnwrap(ProviderIdentity(restoring: canonicalProviderA))
        for _ in 0..<64 {
            XCTAssertNotEqual(ProviderIdentity(), restored)
        }
    }
    ...
    func testSerialization_RoundTripsExactly() throws {
        let original = ProviderIdentity()
        let firstRestore = try XCTUnwrap(ProviderIdentity(restoring: original.canonicalString))
        XCTAssertEqual(firstRestore, original)
        XCTAssertEqual(firstRestore.canonicalString, original.canonicalString)

        let secondRestore = try XCTUnwrap(ProviderIdentity(restoring: firstRestore.canonicalString))
    ...
    func testSerialization_CanonicalFormIsStable() throws {
        let a = try XCTUnwrap(ProviderIdentity(restoring: canonicalProviderA))
        for _ in 0..<8 {
            XCTAssertEqual(a.canonicalString, canonicalProviderA)
        }
    }
    ...
    func testSerialization_MalformedInputIsRejected() {
        ...
        for value in malformed {
            XCTAssertNil(ProviderIdentity(restoring: value), "Expected rejection of \(value)")
        }
    }
    ...
    func testSerialization_CodableRoundTripsExactly() throws {
        ...
        let decoded = try JSONDecoder().decode(ProviderIdentity.self, from: data)
    ...
    func testSerialization_CodableRejectsMalformedValues() {
        ...
        for json in malformed {
            ...
            XCTAssertThrowsError(try JSONDecoder().decode(ProviderIdentity.self, from: data), json)
        }
    }
